...
- **组合风控三参数（2026-09-04 负责人裁定预注册；参照交易实践的「组合总热度/
  相关集群/回撤节流」原则）**——paper 期**仅记录观测、不拦截**（runs.summary
  的 `risk_gates` 键：每比赛日落一次），**实盘入场硬前提**（任一未生效则不得
  真金下注，§10 M5 入场前提并列表）：
  1. **总敞口上限**：单轨在途注金合计（pending stake）≤ **30%** 该轨 bankroll
     ——落注不冻结余额的语义下，这是唯一的组合级总量闸
  2. **同场上限**：同场同轨在途 ≤ **2 注**（同场多市场是相关赌注——H 与 O2.5
     依赖同一比分进程；参照相关品种集群限额）
  3. **回撤节流**：单轨 bankroll 自峰值回撤 > **20%** 时，单注上限 2% 减半为
     1%（峰值 `paper_bankroll_peak:{strategy}` 随结算维护，anti-martingale）
- 固定赔率无止损/调仓（下注即锁死，风险=全部注金）——上述参数正是对该特性的
  组合级补偿；分散度受赛程结构性限制（周末集中），属已知且接受
