...
- **M1（数据层 + 回测基建）已完成**（2026-09-03）：五大联赛 34 赛季 59,176 场全量入库、幂等、抽样对账通过——详见 `docs/m1-report.md`（含终审结论、计划偏差裁决摘要与 **M2 携带项**）
- 下一步是 **M2（模型 + 回测评估）**——go/no-go 关卡：walk-forward log-loss 相对去水收盘基准劣化 ≤1% 才继续，否则项目止步于研究结论
