...
- **M2（模型 + 回测评估）完成，判决 NO-GO**（2026-09-03）：分层 Dixon-Coles 的 walk-forward log-loss 劣化 **+3.02%**（判据 ≤1%），11,605 场 / 2019-2025 七赛季，half-life 50/200/400 与 σ 0.5/0.8/1.5/3.0 双轴扫描全部 NO-GO（机理：过度收缩；σ 轴进一步证明弥合离散度反而使 log-loss 更差，见判决书补充节）——`docs/m2-verdict.md`（含复现命令、校准表、§8.3 模拟盘分解附录；判决经独立逐位复现审查确认）
