...
st.header("B 线 · A/B 双轨进度（§12.3 预注册判据）A/B Tracks")
try:
    t = ab_tracks()
...
for col, (name, tr) in zip((c1, c2), t.items()):
    with col:
        ...
        st.metric("ROI（已结算）Settled", "—" if tr["roi"] is None else f"{tr['roi']:+.1%}")
        st.metric("CLV 中位数 Median", "—" if tr["clv_median"] is None
...
if any(tr["cum"]["dates"] for tr in t.values()):
    ...
    st.info("两轨均无已结算注——累计 P&L 曲线待首个结算日落库后出现 / No settled bets yet")
