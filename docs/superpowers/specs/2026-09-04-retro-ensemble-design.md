...
### 11.1 活库首批周批（2026-09-04，batch #1，divergence top10+对照5，N=3，45 次 hermes 调用）

| 项 | 实测 | 解读（预写规则对照） |
|---|---|---|
| 聚合 ok | 11/15 场（error 4 = 全员失败场） | 成员失败 26/45=58%——远差于 E2E 单测 67%，负载下失败率待归因（成员失败原因无落库列，spec §15 待办加重） |
| 三档（含 ⚠ 修正） | 全同 10（**其中 k=1 退化 4 场**）/多数 0/无多数 1 | 真三员全同 6 场、k=1 4 场、无多数 1 场——k=1 ⚠ 行首批即命中其设计目的；一致率 91% 是乐观上界 |
| audit | **违规 8/8=100%，全部「零证据」选赛前成因标签** | 与 S0+S1 E2E（1/1）独立复现：**证据义务失守是系统性的**——归因「自信但零出处」。放量裁定（spec §15 待负责人）的证据已从个案变为两批复现 |
| 单场耗时 | ≈46s（3×15s） | 与预估同带；逐场 commit 热修后长锁解除（agentline 并发批曾撞 locked，已修 pipeline.py） |
| 运维缺陷（新发现） | 活库 schema_version=6 却缺 attributor 列（版本戳跑在形状前——多分支并行 bump 所致） | 已手工修复；**根治需 init_db 增加形状校验**（版本号不再作为唯一信任源），列为下一项工程任务 |
