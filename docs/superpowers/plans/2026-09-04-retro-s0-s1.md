...
def test_ok_path(monkeypatch):
    ...
    def fake_run(argv, **kw):
        ...
        # 勘误（2026-09-04 实施）：原稿把载荷放在第 4 位置参（stderr）——
        # CompletedProcess(args, rc, stdout, stderr)，载荷应在 stdout（第 3 位），
        # hermes oneshot 的最终文本实测走 stdout。以仓库 tests/retro/test_runner.py 为准。
